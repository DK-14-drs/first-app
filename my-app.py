...
with st.form("survey_form"):
    ...
    street = st.selectbox("가장 좋아하는 우리 동네 거리 이름을 선택하세요", ["중앙로","청담로","강남대로","홍대앞거리","그 외"])

    cleanliness = st.slider("해당 거리를 얼마나 깨끘하다고 느끼나요?(1 매우 불만족 ~ 5 매우 만족)", min_value=1, max_value=5, value=3)
...
if submitted:
    ...
    if not os.path.isfile(DATA_PATH):
        ...
        df = df.to_csv(DATA_PATH, index=False, encoding="utf-8-sig")
